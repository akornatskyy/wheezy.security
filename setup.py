...
def module_name_from_src_path(path: str) -> str:
    ...
    early, which can produce incorrect module names (e.g. "security.*").
    """
...
def optional_cython_extensions() -> object:
    ...
    package_root = os.path.join("src", "wheezy", "security")

    sources: list[str] = []
    sources.extend(glob(os.path.join(package_root, "*.py")))
    sources.extend(glob(os.path.join(package_root, "crypto", "*.py")))
