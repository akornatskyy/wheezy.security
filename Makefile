...
upload:
	...
	sed -i "s/'0.1'/'0.1.$$REV'/" src/wheezy/security/__init__.py ; \
...
test:
	$(PYTEST) -q -x --pep8 --doctest-modules src/wheezy/security

nose-cover:
	$(NOSE) --stop --with-doctest --detailed-errors \
		--with-coverage --cover-package=wheezy.security

test-cover:
	$(PYTEST) -q --cov-report term-missing \
		--cov wheezy.security src/wheezy/security/tests
